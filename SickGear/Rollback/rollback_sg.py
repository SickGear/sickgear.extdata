...
class ImageRollback(object):
    ...
    def downgrade_old_naming(self):
        ...
        try:
            ...
            """ deprecated_item, remove in 2020 """
            # noinspection PyPep8Naming
            from sickgear.helpers import moveFile as move_file
        try:
            from sickgear.indexers.indexer_config import INDEXER_TVDB as TVINFO_TVDB, INDEXER_TVRAGE as TVINFO_TVRAGE
        except ImportError:
            from sickgear.indexers.indexer_config import TVINFO_TVDB, TVINFO_TVRAGE
...
class ConfigFile(RollbackBase):
    ...
    def rollback_v20(self):
        ...
        if growl_host:
            ...
            if 2 == len(h):
                ...
                new_host = h[0]
        if new_pass:
            sickgear.CFG['Growl']['growl_password'] = self.encrypt(new_pass)

        sickgear.CFG['Growl']['growl_host'] = new_host

        sickgear.CFG['General']['config_version'] = 19
        sickgear.CFG.write()
        sickgear.GROWL_HOST = new_host
        sickgear.GROWL_PASSWORD = new_pass

    # Encryption Functions
    def encrypt(self, data, do_decrypt=False):
        encryption_version = self.check_setting_int(sickgear.CFG, 'General', 'encryption_version', 0)
    ...
    def run(self, rollback_version, raise_exception=False):
        ...
        try:
            c_version = self.check_setting_int(sickgear.CFG, 'General', 'config_version', 0)
...
class MainDb(DBRollbackBase):
    ...
    def rollback_100008(self):
        ...
        for t in ('castlist', 'characters', 'character_ids', 'persons', 'person_ids', 'character_person_map',
                  'tv_src_switch', 'switch_ep_result', 'character_person_years'):
